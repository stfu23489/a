...
local function printFailedPlayers(violationLevels)
...
local function FlightACheck()
    ...
    local function checkFlightA()
        while true do
            ...
            printFailedPlayers(violationLevels)
        end
    end
    ...
    local coFlightA = runCoroutine(checkFlightA)

    while coroutine.status(coFlightA) ~= "dead" do
        wait(0.1)
    end
end
...
local function FlightBCheck()
    ...
    local coFlightB = runCoroutine(checkFlightB)

    while coroutine.status(coFlightB) ~= "dead" do
        wait(0.1)
    end
end

print("Running Flight Checks")
FlightACheck()
FlightBCheck()
